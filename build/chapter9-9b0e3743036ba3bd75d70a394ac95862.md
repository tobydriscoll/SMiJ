...
Now that you're becoming comfortable with Julia basics, it's a good time to say a bit more about its type system. For example, we've worked with Toeplitz matrices several times. You might be surprised that a `Toeplitz` matrix isn't exactly a matrix in Julia:
