...
:icon: false
:class: dropdown
I used `interpolate=true` in the `heatmap` call to get a smoother-looking plot. I guess that's a bit hypocritical given my earlier stances on keeping spectral accuracy, but interpolating faithfully to 4th order in time is not easy here. The proper way to do it would be to use a solver from the [DifferentialEquations](https://docs.sciml.ai/DiffEqDocs/stable/) package, which provides interpolation in time. It even offers [exponential Runge–Kutta methods](https://docs.sciml.ai/DiffEqDocs/stable/solvers/ode_solve/#exp_RK).
