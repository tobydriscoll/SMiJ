...
For the 3-D Laplacian, your equivalent of the `matvec` function has to reshape a vector input to an $(N-1) \times (N-1) \times (N-1)$ array, apply the 3-D Laplacian to it (for which `mapslices` is very handy), and then reshape the result back to a vector. 
