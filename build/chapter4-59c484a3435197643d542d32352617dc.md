...
::::{exercise}
:label: exr4-length
Choosing $N$ so that at least 5 digits are accurate for the first 4 eigenvalues, plot the first 4 eigenvalues as functions of the domain radius `L` for `L` in the range from 1 to 10. What scales (log/linear) are best on each axis?
::::