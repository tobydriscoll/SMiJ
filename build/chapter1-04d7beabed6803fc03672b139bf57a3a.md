...
In these early chapters, I've put details into collapsible notes to help you get around.
...
> In these sections for each chapter, I give some tips on adapting the original exercises from *SMiM* for Julia and suggest one or more new ones.

:::{tip} *SMiM* Exercise 1.4

You can use `@timed` from base Julia to get the time taken by a line of code. But you will get more accurate results from installing the `BenchmarkTools` package, which averages over multiple calls:
...
::::{exercise}
:label: exr1-order2
Modify [Program 1](#p1) to use the second-order finite difference formula (1.1) from *SMiM* instead of the fourth-order one. Be sure to modify the dashed line and its label as well.
::::

::::{exercise}
:label: exr1-extrapolate
When the leading error term of a method is known, multiple results from the method can be used to extrapolate to a more accurate result. Suppose the fourth-order finite difference method $F(n)$ has an error expansion of the form

:::{math}
F(n) = d + c_4 n^{-4} + c_6 n^{-6} + \cdots,
:::

where $d$ is the true value of the derivative and $c_4$ and $c_6$ are unknown constants. Show how to combine $F(n)$ and $F(2n)$ to get an approximation to $d$ with sixth-order accuracy. Implement this idea in a modification of [Program 1](#p1) and demonstrate sixth-order accuracy.
::::
