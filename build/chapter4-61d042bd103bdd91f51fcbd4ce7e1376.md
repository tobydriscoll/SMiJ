...
One of my favorite perks in Makie is the `linkaxes!` function, which tells the renderer to give all the linked axes the same limits. That's used to great effect in the output (like in the original), where it's instantly clear how much slower convergence is in the top two cases:
