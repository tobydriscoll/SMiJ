...
In a break with *SMiM*, I've put the Fourier discretization setup into its own function in `SpectralMethodsTrefethen`, based on the formulas in this chapter:
...
Although we won't always need both differentiation matrices at once, the cost of constructing them is negligible next to our applications. When we don't need the second derivative matrix, for example, we can call `x, D, _ = fourier(N)` to ignore the third output.
...
:icon: false
:class: dropdown
I used a superscript for the name of the second derivative matrix by typing `D\^2`, then {kbd}`TAB`. However, as the text points out, this matrix is not actually the square of the first derivative matrix.
::::
...
By now you should be seeing some familiar motifs in [Program 4](#p4). But there's new stuff to look at, too. One fun little refinement is the use of `MultiplesTicks(5, π, "π")` to request 5 ticks at multiples of $\pi$, with tick labels that have `π` appended to them.
...
Remember that arguments after the semicolon must be keyword arguments. We're taking advantage of a shorthand for these arguments. Consider line 12, for example:
...
Line 15 introduces `scatterlines!`, which combines a scatter plot (markers) with a lines plot (connecting segments).
::::
...
In line 25, the `align` keyword is given a tuple of two values whose names each start with a colon, `:`. This is the syntax for creating a {term}`symbol`. You can think of a symbol as a non-numeric constant, or as a string that is not meant to be printed.
::::
...
Our first PDE solution! But the output here is a bit unlike Output 6 in *SMiM*. The `waterfall` plot in MATLAB is not natively in Makie. Rather than try to reconstruct it, I have opted to use a `heatmap` instead, which conveys the result equally well, if a bit less lusciously.
...
This view of the result clearly exposes a flaw in the solution—more on that in the next section.
...
::::{note} Horizontal concatenation
:icon: false
:class: dropdown

In line 24, I use square brackets to construct an array, with horizontal concatenation implied by the space. In Julia, vectors are one-dimensional arrays, but when a vector is used in a matrix context, it is treated as a column vector. Thus, the result of `[v zeros(N, ntime)]` is an array whose first column is `v` and whose remaining `ntime` columns are zeros. You could also use `hcat` or `cat` with `dims=2` for horizontal concatenation.
::::
...
Beyond the switch to animation, there is a change in substance here. You can see in [Output 6](#output6) that the solution looks pixelated in both time and space. It's easy to get more values in the time variable: just decrease the `plotgap` defined in [Program 6](#p6). But what should be done in space? The easy answer is to rely on a linear or perhaps cubic spline interpolation of the values. That might produce a reasonable plot, but you've discarded most of the high accuracy that the Fourier spectral method provided for the discrete solution.

This situation arises a fair amount for spectral methods, which are meant to work well at resolutions that seem too coarse visually. The best way to handle off-grid evaluation is to interpolate spectrally—which was, after all, a major point of this chapter. I've created the `fourinterp` function in `SpectralMethodsTrefethen` for that job, based on (3.7)–(3.8) for even $N$ only.
...
To convert a vector `v` of grid values into a callable interpolating function, we can now use `fourinterp(v)`.
...
:icon: false
:class: dropdown
The animation mechanism in Makie is to define {term}`Observable` quantities whose changing values drive the animation. In [Program 6-anim](#p6anim), there are two Observable values: scalar `time` and the discrete solution `v`. Other variables whose values are derived from an Observable quantity should be defined using the `@lift` macro, as in line 21 to define `title`. Plot objects created using Observables will be updated whenever the underlying values change. Thus, our `lines` object in lines 23–24 depends on `u` and `title`, which in turn depend on `v` and `time`.
...
:icon: false
:class: dropdown
In line 23 I introduce a tidy way to plot most functions without providing an explicit discretization of them. The syntax `lines(a..b, f)` will automatically select points in the interval $[a,b]$ to produce a smooth-looking plot of the function $f$. This is analogous to `fplot` in MATLAB.
::::
...
For 3.5 in *SMIM*, you can use `@timed` to get the time taken by a command, like so:
...
```{code-cell}
stats = @timed rand(2000, 2000)
println("Time taken: ", 1000stats.time, " milliseconds")
```

You get more accurate results from installing the `BenchmarkTools` package, which will average over multiple calls:

```{code-cell}
using BenchmarkTools
stats = @btimed rand(2000, 2000)
println("Time taken: ", 1000stats.time, " milliseconds")
```

::::{exercise}
:label: exr3-fft
Following the instructions for Exercise 3.5 in *SMiM*, compare the timing results for `fft` and `rfft`.
::::

::::{exercise}
:label: exr3-lferror
Determine how the size of the wrong-direction wave error in [Output 6](#output6) and [Output 6-anim](#output6anim) depends on $\Delta t$. What do you suspect causes this error? Can you find a way to fix it, i.e., make it $O(\Delta t^2)$ as expected for leap frog?
::::
