...
`NaN` is a special floating-point value, and all floating-point values are of type `Number`, so the assertion is true. To test for a `NaN` value, you can use the `isnan` function.

:::{caution}
Numerical operations and comparisons involving `NaN` are always `NaN`. Most notoriously,
