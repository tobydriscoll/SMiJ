...
::::{exercise}
:label: exr2-gibbs
Rerun [Program 3](#p3) for $h=0.5$ and $h=0.25$. You may want to add the argument `markersize=4` to the `scatter!` call to make the points smaller. (As is pointed out on page 15 of *SMiM*, the oscillations around the step function values in the middle plot get narrower but not shorter.)