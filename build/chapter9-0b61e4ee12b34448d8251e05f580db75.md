...
We've worked with Toeplitz matrices several times. You might be surprised that a `Toeplitz` matrix isn't exactly a matrix in Julia:
...
We've multiplied Toeplitz matrices with vectors, too. Julia has a way of indicating that an object is matrix-like, even when it is not truly represented by a contiguous block of all its values in memory:

:::{code-cell}
A isa AbstractMatrix
:::

Being subtyped from `AbstractMatrix`, the object `A` promises that it can do the basic things a matrix should do. But behind the scenes, special algorithms can be used to save time and memory.
