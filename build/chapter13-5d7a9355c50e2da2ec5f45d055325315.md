...
In this program, we exploit the explicit time-stepping method (Euler) to leave the boundary values unchanged as the solution evolves.
...
There is a bit of subtlety involved. The first and last rows of the matrix `D²` are zeroed out, and the fact that the boundary values are steady-state solutions ensures that the boundary values remain unchanged. 
