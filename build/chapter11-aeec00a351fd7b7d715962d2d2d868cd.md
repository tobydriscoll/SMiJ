...
I found the outputs for this chapter in *SMiM* to be a bit coarse-looking, so I have interpolated the results onto a finer polar grid. Note that I chose to plot using the parameterization $r \in [-1, 1]$ and $\theta \in [0, \pi]$, which makes it bit easier to deal with the origin.
