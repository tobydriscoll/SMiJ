...
This situation arises a fair amount for spectral methods, which are meant to work well at resolutions that seem too low visually. The best way to handle off-grid evaluation is to interpolate spectrally—which was, after all, a major point of this chapter. I've created the `fourinterp` function in `SpectralMethodsTrefethen` for that job, based on (3.7)–(3.8) for even $N$ only.
