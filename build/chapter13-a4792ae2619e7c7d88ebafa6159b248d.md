...
For an eigenvalue problem (for which only homogeneous conditions are sensible mathematically), we can use a generalized formulation $Av=\lambda Bv$ in which two rows of $B$ are zero and the corresponding rows of $A$ are replaced by the boundary operators. This will put two generalized eigenvalues at infinity that can be ignored.
