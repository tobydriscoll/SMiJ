...
One of my favorite perks in Makie is the `linkaxes!` function, which tells Makie to give all the linked axes the same limits. That's used to great effect in the output (like in the original), where it's instantly clear how different convergence can be in four archetypical cases:
