...
For the animated form[^notpackaged] of [Program 27](#p27anim), I use the [`ETDRK4` time integrator](https://docs.sciml.ai/DiffEqDocs/stable/solvers/ode_solve/#exp_RK), which implements the fourth-order exponential Runge–Kutta method. Formulating the method of lines as a `SplitODEProblem` allows us to separate the stiff linear term, which is handled implicitly, from the nonlinear term of the PDE. The resulting solution object can be called at any value of $t$ to get the solution in Fourier space.
...
::::{tip} *SMiM* Exercises 10.6, 10.7, 10.8
