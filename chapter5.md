...
In *SMiM*, the notational conventions shifted a little bit for this code. Rather than using $u$ to mean a function with grid values called $v$, `p9.m` uses the variable $u$ to mean grid values of a polynomial $p$. That's consistent with the math in this chapter, but not with the code in the preceding ones, and I've opted in [Program 9](#p9) to stay with `u` to mean the function whose grid values are `v`. We'll see that in future programs as well.

On a subtle but important mathematical note, you would find big differences in errors between my [Program 9](#p9) and the original in *SMiM* at larger values of $N$. The `polyfit` function in MATLAB (at this writing time) computes the coefficients of the interpolating polynomial in the monomial basis, which is a bad choice due to its poor conditioning. The algorithm in line 15 above, though, uses a better-conditioned approach from [](doi:10.1137/19M130100X). Of course, while that computes the interpolating polynomial accurately, the polynomial is not necessarily an accurate representation of the underlying function when the nodes are equally spaced!
...
:::{important}
Unlike in MATLAB, coefficients should be given in order of increasing degree:
